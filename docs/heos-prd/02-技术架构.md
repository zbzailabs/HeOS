...
Prisma/Neon 属于初始化示例，不作为一期业务默认数据底座。当前一期业务表通过 D1 migrations 建立，覆盖权限、标准字典、遥测、项目资产、告警、农事、追溯、AI 记录、同步记录和 AI provider 指标。
...
图片、报告、追溯素材和导出文件使用 Cloudflare R2。业务表只保存对象 key、元数据、来源和归属关系。当前代码已建立追溯导出计划边界，生成稳定 R2 object key 和 `trace.export` 审计动作；真实文件生成、上传和回填对象元数据进入后续交付。
...
生产运维使用 `/api/operations/health` 获取只读健康快照。检查内容包括 D1 核心表计数、最近 Renke 同步、最新遥测时间、开放告警数量和 AI provider 24 小时窗口。该接口用于持续核验生产状态，不承担业务写入。
