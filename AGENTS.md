...
项目新任务开始先读取 HeOS 本身文档、GitHub Issues 和 GitHub Projects；每次任务结束，在对应 Issue 和 Project 中记录工作内容。
...
人类负责提出需求、大方向、问题反馈和验收；AI 负责拆解任务、创建 Issue、维护 Projects、编写代码、验证交付和更新 HeOS 文档记录。See `docs/agents/人类与AI协作方式.md`.
...
- HeOS 本身文档作为长期知识库，记录产品规格、架构设计、开发指南、验收标准、ADR 和阶段性报告。
- 新任务开始时先读取 `AGENTS.md`、`DESIGN.md`、`CONTEXT.md`、`docs/adr/`、`docs/specs/`、`docs/heos-prd/` 和 GitHub 当前任务；不存在的可选文档按现状静默跳过。
- 在 GitHub Issues 建立或确认任务，补齐规格、边界、验收标准和验证命令。
- 开发执行围绕 GitHub Issue 进行，PR、commit、验证记录必须关联对应 Issue。
- 任务状态流转在 GitHub Projects 中维护。
- 任务结束时，将 PR 链接、commit、完成内容、验证命令与结果、未完成项写入对应 Issue，并更新 GitHub Projects 状态。
- 涉及长期决策、产品边界、架构取舍或验收口径变化时，同步更新 HeOS 仓库内对应文档。

执行优先级为：GitHub Issues = 任务事实来源；GitHub Projects = 开发看板与排期；HeOS 仓库文档 = 长期知识库和交付记录。
